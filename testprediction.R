...
### Random forest for above
# HIV Model
library(randomForest)
t1 = Sys.time()
rf_hiv = randomForest(hiv_test ~  material
                      + workshops + hiv_known 
                      + sw_birthyr
                      + swperatpd  + density + first_quart.x 
                      + homesite + district,
...
rfpred_hiv = predict(rf_hiv, newdata = testdata, type = "")
head(rfpred_hiv)
plot(rfpred_hiv~melpred_hiv)
abline(v = 0.5)
abline(h = 0.5)

library(ROCR)
perf_hiv = prediction(rfpred_hiv, testdata$hiv_test)
auc_hiv = performance(perf_hiv, "auc")
auc_hiv
auc_val_hiv = auc_hiv@y.values; auc_val_hiv
pred3_hiv = performance(perf_hiv, "tpr","fpr")
...
library(caret)
### CHECK if threshold = 0.5 is correct
rfpred_hiv_class = rep(0, length(rfpred_hiv))
rfpred_hiv_class[which(rfpred_hiv>0.5)] = 1 
xtab = table(rfpred_hiv_class, testdata$hiv_test)
conf_hiv = confusionMatrix(xtab)
conf_hiv
...
accuracy_hiv = sum(diag(mat_hiv))/sum(mat_hiv); accuracy_hiv
precision_hiv = diag(mat_hiv)/rowSums(mat_hiv); precision_hiv
recall_hiv = diag(mat_hiv)/colSums(mat_hiv); recall



# STD Model
t1 = Sys.time()
rf_std = randomForest(std_test ~ numvisits + numservices + material
                      + workshops + hiv_known + sw_birthyr 
                      + swperatpd + density + stdtest_budg + first_quart.x 
                      + homesite + district,
                      data = traindata)
t2 = Sys.time()
t2-t1 #6 minutes
...
rfpred_std = predict(rf_std, newdata = testdata, type = "")
head(rfpred_std)
plot(rfpred_std~melpred_std)
abline(v = 0.5)
abline(h = 0.5)


library(ROCR)
perf_std = prediction(rfpred_std, testdata$std_test)
auc_std = performance(perf_std, "auc")
auc_std
auc_val_std = auc_std@y.values; auc_val_std
pred3_std = performance(perf_std, "tpr","fpr")
plot(pred3_std,main="HIV Test: ROC Curve for Hierarchical Logistic Model",col=2,lwd=2)
abline(a=0,b=1,lwd=2,lty=2,col="gray")
...
#TB
t1 = Sys.time()
rf_tb = randomForest(tb_test ~ numvisits + numservices + material
                     + workshops + hiv_known + sw_birthyr 
                     + swperatpd + density + tbtest_budg + first_quart.x 
                     + homesite + district,
                     data = traindata)
t2 = Sys.time()
t2-t1 #6 minutes
...
rfpred_tb = predict(rf_tb, newdata = testdata, type = "")
head(rfpred_tb)
plot(rfpred_tb~melpred_tb)
abline(v = 0.5)
abline(h = 0.5)


library(ROCR)
perf_std = prediction(rfpred_tb, testdata$tb_test)
auc_std = performance(perf_std, "auc")
auc_std
auc_val_std = auc_std@y.values; auc_val_std
pred3_std = performance(perf_std, "tpr","fpr")
...
# See vignettes too
# set.seed(123)
# y = MCMCglmm(hiv_test ~ numvisits + numservices + material
# + workshops + hiv_known + sw_birthyr
# + swperatpd + density + budgetperperson + first_quart.x, random = ~homesite, 
# family = "categorical", data = traindata, prior = prior1)
# summary(y)
# # Very wide confidence intervals - look into setting priors
